...
class OpenAITokenizer
{
    ...
    public static function truncate(string $text, int $tokenCount): string
    {
        ...
        $truncatedTokens = array_slice($tokens, 0, $tokenCount);
        $truncatedText = self::decode($truncatedTokens);

        return $truncatedText;
    }

    // ... (include all the functions from the code you provided here) ...
    public static function gpt_utf8_encode(string $str): string
    {
        $str .= $str;
        $len = \strlen($str);
    ...
    public static function encode($text)
    {
        ...
        foreach ($bpe_merges_temp as $bmt) {
            ...
            $split_bmt = array_filter($split_bmt, function ($x) {
                return self::gpt_my_filter($x);
            });
    ...
    public static function gpt_my_filter($var)
    {
    ...
    public static function gpt_dictZip($x, $y)
    {
        $result = [];
        $cnt = 0;
        foreach ($x as $i) {
    ...
    public static function gpt_indexOf($arrax, $searchElement, $fromIndex)
    {
        $index = 0;
